...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  \item \textbf{Affordable Learning:} Allowing students to learn practical skills at affordable rates through one-on-one sessions and demonstration videos.
  ...
  \item \textbf{Fair Transaction Handling:} Using an escrow-style simulated digital wallet to ensure that funds are held securely until sessions are completed.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  ...
  \item \textbf{Implement an Escrow-Style Simulated Wallet:} To create a transaction mechanism that holds session fees in escrow upon booking and transfers payment to the mentor after the session is conducted.
...
\textbf{Payment Handling} & Upfront course fee & Milestone / project fee & Dual-ledger escrow-style wallet released on session confirmation \\ \hline
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  \item \textbf{Escrow-Style Simulated Digital Wallet:} A transaction mechanism where booked session fees are held in escrow and credited to the mentor's wallet only after the student confirms session completion.
\end{itemize}
...
\end{itemize}
Based on the identified development requirements, the project is economically feasible for an academic prototype.
...
\end{itemize}
The proposed interface and workflows are designed to support adoption by both seniors and students.
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  \item \textbf{Earnings Overview:} Displays completed sessions, escrowed payments, and available wallet balance.
\end{itemize}
...
\subsection{Escrow-Style Wallet and Financial Settlement Module}
The Escrow-Style Wallet Module manages session payments and ensures financial transparency:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Dual-Ledger Wallet Accounts:} Every registered user has a dedicated simulated wallet record maintaining available and held balances.
  ...
  \item \textbf{Hold on Unconfirmed Sessions:} If a session is cancelled, funds remain held until status is updated.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{User Account Oversight:} Admins can review registered user accounts and monitor platform activity.
  \item \textbf{Content Moderation:} Monitors published courses, videos, and student reviews to ensure community safety standards.
  \item \textbf{Platform Monitoring:} Reviews course listings and monitors wallet transaction logs.
\end{itemize}
...
\texttt{password} & VARCHAR(255) & NOT NULL & Password hash stored instead of plain text \\ \hline
...
\texttt{balance} & DECIMAL(10,2) & DEFAULT 0.00 & Available balance for session transactions \\ \hline
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Senior Mentor Actor:} Engages with use cases including Register/Login, Create Skill Course, Upload Video Content, Manage Availability, Confirm Sessions, and View Wallet Balance.
  ...
  \item \textbf{Administrator Actor:} Interacts with Review User Accounts, Moderate Courses, and Monitor Platform Activity.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{High-Contrast Typography:} Clear typography with readable font sizing and strong color contrast, designed with reference to WCAG 2.1 AA accessibility recommendations \cite{wcag21}.
...
\begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  ...
  \item \textbf{Escrow-Style Simulated Wallet Architecture:} The dual-ledger digital wallet holds funds in escrow, releasing session payments to the mentor after verified service delivery.
...
\textbf{Interface Accessibility} & Designed for high contrast & Clear fonts and high contrast assist elderly users \cite{wcag21}. \\ \hline
